...
class Attention(nn.Module):
    ...
    def forward_yi(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config):
        ...
        self.feed_forward = FeedForward(
            dim=self.dim, hidden_dim=11008, multiple_of=config['multiple_of']
        )
...
class YITransformer(nn.Module):
    ...
    @torch.inference_mode()
    def forward_yi(self, tokens: torch.Tensor, start_pos: int):
...
yi_default_config = {"dim": 4096, "multiple_of": 256,
                                "n_heads": 16, "n_layers": 28, "norm_eps": 1.0e-6,
                                "vocab_size": -1, "first_layer": 27, "kv_heads": 16}
model = YITransformer(yi_default_config)
