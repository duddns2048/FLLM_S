...
from mistral import MISTRALTransformer
import time
...
class VisionTransformerInput(nn.Module):
    def __init__(self, image_size, patch_size, in_channels, embed_size):
        ...
        self.llm = MISTRALTransformer(llm_default_config)

        # Load LLM checkpoint for the encoder layer
        llm_path = "./mistral-7B"
        logger.info("Loading MISTRAL checkpoints")
...
# python VIT_MISTRAL.py --data_dir /path/to/data --data_file /path/to/data_file.txt --image_size 128,128,128 --patch_size 16,16,16 --epochs 100 --batch_size 4 --lr 3e-4 --task_name 'Task01_BrainTumour'