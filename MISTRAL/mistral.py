...
class Attention(nn.Module):
    ...
    def forward_mistral(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
...
class MISTRALTransformer(nn.Module):
    ...
    @torch.inference_mode()
    def forward_mistral(self, tokens: torch.Tensor, start_pos: int):
...
# mistral_default_config = {"dim": 4096, "multiple_of": 256,
#                                 "n_heads": 32, "n_layers": 32, "norm_eps": 1.0e-5,
#                                 "vocab_size": -1, "first_layer": 31, "kv_heads": 8}
# model = MISTRALTransformer(mistral_default_config)
