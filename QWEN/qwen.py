...
class Attention(nn.Module):
    ...
    def forward_qwen(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config):
        ...
        self.feed_forward = FeedForward(
            dim=self.dim, hidden_dim=18944, multiple_of=config['multiple_of']
        )
...
class QWENTransformer(nn.Module):
    ...
    def custom_load_state_dict(self, checkpoint, tail=False, strict=False):
        if tail:
            for i in range(self.first_layer, self.n_layers):
                # Adjust to match the correct key format
                ...
                # Create a dictionary to hold the layer-specific weights
                layer_checkpoint = {k: checkpoint[f'model.layers.{i}.{k}'] for k in layer_checkpoint_keys}               
    ...
    @torch.inference_mode()
    def forward_qwen(self, tokens: torch.Tensor, start_pos: int):
...
# tokens = torch.randn(1, 512, 3584)  # batch size, sequence length, hidden dimension
# # Initialize the model
# qwen_default_config = {"dim": 3584, "multiple_of": 256,
#                                 "n_heads": 28, "n_layers": 28, "norm_eps": 1.0e-6,
#                                 "vocab_size": -1, "first_layer": 27, "kv_heads": 4}
# model = QWENTransformer(qwen_default_config)
